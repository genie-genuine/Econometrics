...
#============================#
# MAKE DATA
y.t = rv.data$SNP.RV[1:nrow(rv.data)]      # SNP.RV
head(y.t); length(y.t) 
x.t = rv.data$VIX[1:(nrow(rv.data))]   # VIX
head(x.t); length(x.t)
...
ADL.bic; min(ADL.bic)    # ADL(6,6)

## 1-step ahead forecasting
AR8.fore = c(); ADL66.fore = c()
for(i in 3106:(length(y.t)-1)){
  ...
  # ADL(6,6)
  ADL.fit1 = lm(y_t ~                            
                  y_t_1 + y_t_2 +y_t_3 + y_t_4 + y_t_5 + y_t_6 +    
                    x_t + x_t_1 + x_t_2 + x_t_3+ x_t_4 + x_t_5 );
  ADL66.fore[i-3105] = sum(ADL.fit1$coef*c(1, y_t[length(y_t)], 
                                           y_t_1[length(y_t)], 
                                           y_t_2[length(y_t)], 
                                           y_t_3[length(y_t)],
                                           y_t_4[length(y_t)],
...
# 2. 예측력 비교
# AR(8)     VS   ADL(6,6)  
# AR8.fore  VS   ADL66.fore
# MAE
AR8.MAE = mean(abs(AR8.fore - y.t[3107:length(y.t)]))
ADL66.MAE = mean(abs(ADL66.fore - y.t[3107:length(y.t)]))
AR8.MAE < ADL66.MAE      # AR(8) 모형이 MAE 값이 더 작음  
# MSE
AR8.MSE = mean((AR8.fore - y.t[3107:length(y.t)])^2)
ADL66.MSE = mean((ADL66.fore - y.t[3107:length(y.t)])^2)
AR8.MSE < ADL66.MSE      # AR(8) 모형이 MSE 값이 더 작음
result = matrix(c(AR8.MAE, ADL66.MAE, AR8.MSE, ADL66.MSE), nrow = 2, byrow = T)
row.names(result) = c("MAE", "MSE")
colnames(result) = c("AR8", "ADL(6,6)")
result  # AR(8) 모형이 더 잘 예측
