...
tail(cpi.data) # 1960-1999년

## Make data
# 인플레이션계산 - cpi차분 퍼센트(100) 연간(1,4,7,10)
# Inf = (cpi(t)-cpi(t-1))/(cpi(t-1))*100*4, t = 2,...,T
nrow(cpi.data) # 160개의 데이터

cpi_t = cpi.data$CPI[2:nrow(cpi.data)]       # t는 2부터 
cpi_t_1 = cpi.data$CPI[1:(nrow(cpi.data)-1)] # t-1
Inf.value = 4*100*(cpi_t-cpi_t_1)/cpi_t_1    # 인플레이션율
...
y.t = Inf_t - Inf_t_1                 
length(y.t)                                  # 158개의 데이터

# 실업율 데이터
# y.t 데이터 갯수 158에 맞춰주기 위해 t=3부터
...
## ADL(4,4)
ADL.fit2 = lm(y_t ~                               # 인플레이션 변화율 
                y_t_1 + y_t_2 +y_t_3 + y_t_4 +    # CPI
